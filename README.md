...
model
    activations
        tutte le funzioni di attivazione . py
    layers
        random_layer.py -> continene una funzione per inizializzare i pesi del layer
        layer -> contiene la classe layer con proiezione lineare dei vettori e possibilita; di usare le funzioni di attivazione
    losses
        tutte le funzioni di loss . py
...
utils
    data_loader.py -> classe che gestisce i dataset, puo' fare shuffle, train-val split, k-fold split, batch split
