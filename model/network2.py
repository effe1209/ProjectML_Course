  ...
  def __init__(self, layer_dimensions: list[int], layer_activations: list[str], std : float = None):
    ...
    for i in range(len(layer_dimensions)-1):
      ...
      if dinamic_std is None:
        dinamic_std = np.sqrt(2 / (layer_dimensions[i] + layer_dimensions[i+1])) #std based on fan-in and fan-out
        if layer_activations[i] in ['relu']:
          dinamic_std = np.sqrt(2 / (layer_dimensions[i]))
